...
새 프로젝트라면 KSP를 쓴다. AGP 9에서는 KAPT가 `com.android.legacy-kapt`로 밀려났다. → [`android-agp-kgp-and-compiler-plugins.md`](android-agp-kgp-and-compiler-plugins.md)
...
→ [`android-parcelable-vs-serializable.md`](android-parcelable-vs-serializable.md)
...
"이 쿼리가 왜 이런 결과를 주지?"의 답이 대개 거기 있다. → [`gradle-build-problem-debugging.md`](gradle-build-problem-debugging.md)
...
`chapter205`의 `WishRepository`가 이 경계를 잘 지키고 있다. `ViewModel`은 `WishDao`를 모르고 `WishRepository`만 안다. → [`android-repository-single-source-of-truth.md`](android-repository-single-source-of-truth.md)
