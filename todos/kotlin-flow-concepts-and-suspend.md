...
- **한 가지 짚어 둘 점.** 지금 `WishDao`의 메서드에는 `suspend`가 없다.
  ...
  그래서 `WishRepository.addWish`에 `suspend`를 붙여 놓았지만 **실제로는 중단하지 않고 스레드를 막는다.** `ViewModel`이 `Dispatchers.IO`로 감싸야만 동작하는 이유다. `suspend`가 붙었다고 main-safe해지는 것이 아니다. → [`kotlin-coroutine-dispatchers.md`](kotlin-coroutine-dispatchers.md)
...
`emit`과 `collect`가 둘 다 `suspend`다. 그래서 값을 만드는 쪽도, 받는 쪽도 **중단할 수 있다.** 생산자가 느리면 소비자가 기다리고, 소비자가 느리면 생산자가 기다린다(배압, backpressure). 콜백 기반 스트림에는 없는 성질이다. → [`kotlin-coroutines-continuation-state-machine.md`](kotlin-coroutines-continuation-state-machine.md)
...
**아무도 "다시 조회해"라고 말하지 않는다.** `chapter205`에서 항목을 지우면 목록이 저절로 사라지는 이유다. → [`android-room-architecture.md`](android-room-architecture.md)
...
`stateIn`이 **콜드 `Flow`를 핫 `StateFlow`로 바꾼다.** 화면이 회전해도 쿼리가 다시 돌지 않고, 항상 마지막 값을 갖고 있다. `chapter205`처럼 `collectAsState`로 바로 받으면 이 이점이 없다. → [`compose-collectasstate-flow-to-state.md`](compose-collectasstate-flow-to-state.md)
