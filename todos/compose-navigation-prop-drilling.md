...
필요한 것만 받으면 **재구성 범위가 좁아진다.** `RecipeState`의 `error`만 바뀌어도 지금은 `RecipeScreen` 전체가 재구성 대상이 된다. → [`compose-recomposition-timing-and-scope.md`](compose-recomposition-timing-and-scope.md)
...
`CompositionLocal`은 **테마, 폰트, `Context`처럼 거의 모든 곳에서 쓰이고 자주 안 바뀌는 값**에 어울린다. 내비게이션 콜백은 그런 성격이 아니다. → [`compose-localcontext.md`](compose-localcontext.md)
...
`RecipeScreen`과 `CategoryItem`은 손대지 않는다. **콜백으로 분리해 둔 것의 값어치가 라이브러리를 갈아탈 때 드러난다.** → [`android-navigation-compose-vs-navigation3.md`](android-navigation-compose-vs-navigation3.md)
