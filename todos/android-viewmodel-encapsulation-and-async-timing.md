...
**부수 효과는 효과 API 안에서 해야 한다.** → [`compose-launchedeffect-and-snapshotflow.md`](compose-launchedeffect-and-snapshotflow.md)
...
`collectAsState` 대신 **`first()`로 한 번만 읽는 것**이 핵심이다. 편집 화면에서는 DB를 계속 구독할 이유가 없다. 계속 구독하면 사용자의 입력을 DB 값이 덮어쓴다. → [`kotlin-flow-concepts-and-suspend.md`](kotlin-flow-concepts-and-suspend.md)
...
그리고 화면을 나가면 이 컴포저블이 컴포지션을 떠나므로 `rememberCoroutineScope()`가 취소된다. **마지막 줄 `snackMessage.value = ""`는 실행되지 않을 수 있다.** → [`kotlin-coroutine-scope-concept.md`](kotlin-coroutine-scope-concept.md)
...
**호출자는 저장이 끝났는지, 실패했는지 알 수 없다.** 예외가 나면 코루틴이 조용히 죽거나 앱이 크래시한다. → [`kotlin-coroutines-exception-handling-try-catch.md`](kotlin-coroutines-exception-handling-try-catch.md)
...
목록 화면용 `getAllWishes`와 편집 화면용 폼 상태가 한 클래스에 섞여 있다. 화면마다 `ViewModel`을 두면 각자의 관심사만 갖는다. `viewModel()`을 각 `composable` 안에서 부르면 **그 화면의 `NavBackStackEntry`에 스코프된다.** → [`android-navigation-backstackentry-savedstatehandle.md`](android-navigation-backstackentry-savedstatehandle.md)

다만 강의 흐름상 하나로 가는 것이라면, 최소한 **폼 상태를 화면 진입 시 초기화**하는 것은 반드시 필요하다. 지금은 그 초기화가 컴포지션 중에 일어나고 있다.

### 저장이 화면 수명보다 오래 살아야 한다면

"저장 중에 화면을 나가도 저장은 끝나야 한다"면 `viewModelScope`도 부족할 수 있다. `ViewModel`이 정리되면 취소되기 때문이다. 주입된 앱 수명 스코프를 쓴다. → [`android-viewmodelscope-launch-necessity.md`](android-viewmodelscope-launch-necessity.md)
