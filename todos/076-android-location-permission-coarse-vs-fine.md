...
Play services가 `@RequiresPermission(anyOf = [...])`로 선언한 것과 같은 논리다. → [`075-android-requires-permission-annotation.md`](075-android-requires-permission-annotation.md)
...
`Boolean` 하나로 표현하면 **"대략만 허용"이라는 중간 상태가 사라진다.** 지금 코드가 그 상태를 놓친 것도 결국 이 모델링 문제다. → [`003-kotlin-sealed-class-and-interface.md`](003-kotlin-sealed-class-and-interface.md)
