...
- **"백그라운드에서 실행하기 위해 `viewModelScope.launch`가 필요하다"** → 이 전제가 틀렸다. **`launch`는 스레드를 바꾸지 않는다.** `viewModelScope`의 기본 디스패처는 `Dispatchers.Main.immediate`이고, 그래서 저 블록 안의 코드는 **메인 스레드에서 시작한다**. 로그를 찍어 보면 바로 확인된다.
  ...
  네트워크 I/O를 실제로 백그라운드에서 돌리는 것은 `launch`가 아니라 **Retrofit/OkHttp**다. OkHttp가 자기 스레드 풀에서 요청을 처리하고, 응답이 오면 코루틴을 다시 메인 스레드에서 재개한다. → [`kotlin-coroutines-continuation-state-machine.md`](kotlin-coroutines-continuation-state-machine.md)
...
`viewModelScope.launch`는 이 중 **2번과 4번**을 한다. 3번은 하지 않는다. → [`kotlin-coroutines-suspend-and-event-loop.md`](kotlin-coroutines-suspend-and-event-loop.md)
...
결과가 필요하면 `async`/`await`를 쓴다. 다만 `ViewModel`에서 UI 상태를 다룰 때는 상태 갱신 쪽이 자연스럽다. `launch`가 즉시 반환한다는 성질은 예외 처리에도 영향을 준다. → [`kotlin-coroutines-exception-handling-try-catch.md`](kotlin-coroutines-exception-handling-try-catch.md)
