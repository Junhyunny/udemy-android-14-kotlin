...
현재 코드처럼 `CounterRepository()`를 컴포저블 안에서 만들면, `ViewModel`이 새로 만들어질 때마다 저장소도 새로 생긴다. 저장소가 데이터의 단일 진실 공급원이어야 한다는 원칙과 충돌한다. 관련 내용은 [`049-android-repository-single-source-of-truth.md`](049-android-repository-single-source-of-truth.md)에 정리했다.
