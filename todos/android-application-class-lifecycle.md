...
그리고 `Graph.wishRepository`가 `by lazy`인 것도 같은 맥락이다. 실제로 쓸 때까지 만들지 않는다. → [`kotlin-by-lazy-delegate.md`](kotlin-by-lazy-delegate.md)
...
`Graph` 같은 수동 컨테이너가 사라진다. → [`android-manual-di-graph-object.md`](android-manual-di-graph-object.md)
