...
`abstract class`인 이유는 **Room이 컴파일 시점에 `WishDatabase_Impl`을 생성**하기 때문이다. 우리가 구현할 것이 없다. → [`061-android-room-dao-code-generation.md`](061-android-room-dao-code-generation.md)
...
하이버네이트에는 이런 것이 없다. **모바일 UI에 특화된 설계**다. → [`057-kotlin-flow-concepts-and-suspend.md`](057-kotlin-flow-concepts-and-suspend.md)
...
**`RoomDatabase` 인스턴스는 비싸다.** 앱 전체에서 하나만 만들어 재사용해야 한다. `chapter205`가 `object Graph`에 담아 둔 이유가 이것이다. → [`050-android-manual-di-graph-object.md`](050-android-manual-di-graph-object.md)
...
→ [`054-kotlin-coroutine-dispatchers.md`](054-kotlin-coroutine-dispatchers.md)
...
`Wish` 엔티티가 UI까지 그대로 올라오는 것은 학습 예제에서는 괜찮지만, 규모가 커지면 DB 스키마 변경이 화면 코드까지 흔든다. → [`049-android-repository-single-source-of-truth.md`](049-android-repository-single-source-of-truth.md)
