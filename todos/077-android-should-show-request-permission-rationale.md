...
- **`context as MainActivity` 캐스팅은 위험하다.** `LocalContext.current`가 항상 `MainActivity`라는 보장이 없다. Compose 프리뷰나 다른 컨텍스트 래퍼에서는 `ClassCastException`이 난다. `as?`와 `?:`, 또는 `findActivity()` 확장을 쓰는 편이 안전하다. → [`001-kotlin-is-operator-and-smart-cast.md`](001-kotlin-is-operator-and-smart-cast.md)
...
`StartActivityForResult`로 띄우면 **사용자가 설정에서 돌아왔을 때 콜백이 오므로** 권한 상태를 다시 확인할 수 있다. → [`074-android-activity-result-api-and-launcher.md`](074-android-activity-result-api-and-launcher.md)
...
→ [`003-kotlin-sealed-class-and-interface.md`](003-kotlin-sealed-class-and-interface.md)
