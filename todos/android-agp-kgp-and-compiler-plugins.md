...
이 문제를 이해하려면 **Gradle, AGP, KGP, 컴파일러 플러그인**이 각각 무엇이고 어떤 순서로 맞물리는지 알아야 한다. 빌드 파일들의 역할은 [`gradle-version-catalog-and-build-files.md`](gradle-version-catalog-and-build-files.md)에, 문제를 찾아간 과정은 [`gradle-build-problem-debugging.md`](gradle-build-problem-debugging.md)에 정리했다.
...
확인 방법은 [`gradle-build-problem-debugging.md`](gradle-build-problem-debugging.md)에 정리했다.
