...
`rememberSaveable` 상태를 액티비티 재생성 후 복원하기 위해서다. `it.id`가 `Long`이므로 조건을 만족한다. → [`android-parcelable-vs-serializable.md`](android-parcelable-vs-serializable.md)
...
**스와이프는 실수하기 쉬운 제스처다.** Material 가이드도 되돌릴 방법을 두라고 권한다. `chapter205`의 삭제는 현재 되돌릴 수 없다. → [`compose-scaffold.md`](compose-scaffold.md)
