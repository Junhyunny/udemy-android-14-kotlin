...
취소되면 중단점에서 `CancellationException`이 던져진다. 이 예외를 삼키면 안 되는 이유는 따로 정리했다. → [`055-kotlin-coroutines-exception-handling-try-catch.md`](055-kotlin-coroutines-exception-handling-try-catch.md)
