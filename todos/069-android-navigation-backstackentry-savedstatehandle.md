...
이 중 두 번째가 특히 중요하다. 컴포저블에서 `viewModel()`을 부르면 **그 화면의 `NavBackStackEntry`에 스코프된 `ViewModel`**이 만들어진다. 화면이 백스택에서 빠지면 `onCleared()`가 불린다. → [`046-compose-viewmodel-function-vs-manual.md`](046-compose-viewmodel-function-vs-manual.md)
...
`Category`에 `@Parcelize`가 붙은 이유가 이것이다. → [`024-android-parcelable-vs-serializable.md`](024-android-parcelable-vs-serializable.md)
...
이러면 `savedStateHandle`도, `@Parcelize`도, `?: Category("", "", "", "")` 방어 코드도 전부 필요 없어진다. → [`068-android-navigation-passing-many-arguments.md`](068-android-navigation-passing-many-arguments.md)
...
`RecipeScreen`은 `NavController`를 모른다. 덕분에 프리뷰와 테스트가 쉽고, 나중에 Navigation 3로 옮겨도 화면 코드는 그대로다. 다만 콜백이 여러 단계 내려가는 문제는 따로 있다. → [`070-compose-navigation-prop-drilling.md`](070-compose-navigation-prop-drilling.md)
...
`savedStateHandle`로 값을 실어 나르는 패턴 자체가 사라진다. → [`073-android-navigation-compose-vs-navigation3.md`](073-android-navigation-compose-vs-navigation3.md)
