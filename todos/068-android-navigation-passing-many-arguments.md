...
- **데이터가 항상 최신이다.** 목록에서 넘겨받은 객체는 목록을 만들 때의 스냅샷이다. 상세 화면에서 다시 조회하면 최신값을 본다. 진실 공급원이 하나로 유지된다. → [`049-android-repository-single-source-of-truth.md`](049-android-repository-single-source-of-truth.md)
...
화면 사이로 데이터가 흐르는 대신, **각 화면이 저장소에서 직접 가져온다.** 화면 간 결합이 사라지고, 어느 화면이든 딥링크로 바로 열 수 있게 된다. → [`049-android-repository-single-source-of-truth.md`](049-android-repository-single-source-of-truth.md)
