...
- **`@RequiresPermission`도 컴파일러 기능이 아니다.** `@DrawableRes`와 같은 계열의 **Lint용 메타데이터**다. 붙여도 컴파일은 통과하고, 안 붙여도 컴파일은 통과한다. → [`015-android-drawable-res-annotation.md`](015-android-drawable-res-annotation.md)

- **`allOf`와 `anyOf`를 혼동하면 안 된다.** 주석 처리된 줄은 `allOf`(둘 다 필요)인데, Play services의 실제 선언은 `anyOf`(둘 중 하나면 됨)다. FLP는 **COARSE만 있어도 동작**하기 때문이다. `allOf`로 선언하면 실제보다 강한 조건을 주장하는 셈이다. → [`076-android-location-permission-coarse-vs-fine.md`](076-android-location-permission-coarse-vs-fine.md)
