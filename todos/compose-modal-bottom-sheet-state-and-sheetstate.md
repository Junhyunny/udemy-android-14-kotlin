...
`val`로 읽기만 하고 바꾸는 코드가 없으므로 **상태일 필요가 없다.** `val isSheetFullScreen = true`면 충분하다. 값이 변하지 않는데 `mutableStateOf`로 감싸면 **"이건 바뀔 수 있다"는 잘못된 신호**를 준다. → [`compose-remember-mutablestate-and-by.md`](compose-remember-mutablestate-and-by.md)
...
반면 **`SheetState`는 ViewModel에 두지 않는다.** 애니메이션 상태는 컴포지션에 묶인 UI 요소이고, 화면 회전 시 다시 만들어져야 한다. → [`android-viewmodel-encapsulation-and-async-timing.md`](android-viewmodel-encapsulation-and-async-timing.md)
