...
- **그런데 더 중요한 질문은 "왜 여기서 디스패처를 지정해야만 하는가"다.** 공식 베스트 프랙티스는 **`ViewModel`이 디스패처를 신경 쓰지 않는 것**이다.
  ...
  스레드를 옮기는 책임은 **함수를 만든 쪽**에 있다. `WishDao`에 `suspend`만 붙이면 Room이 알아서 자기 스레드 풀에서 실행하고, `ViewModel`은 `viewModelScope.launch { }`만 쓰면 된다. **지금 코드는 아래 계층의 부실을 위 계층이 떠안고 있는 구조다.** → [`060-android-room-architecture.md`](060-android-room-architecture.md)
...
디스패처는 **"재개된 코루틴 코드를 어느 스레드에서 실행할지"**를 정하는 정책 객체다. 코루틴 자체가 스레드를 만드는 것이 아니다. → [`051-kotlin-coroutines-suspend-and-event-loop.md`](051-kotlin-coroutines-suspend-and-event-loop.md)
...
**함수 안에서 스레드를 옮길 때는 `withContext`, 새 작업을 띄울 때는 `launch`다.** 지금 코드의 `launch(Dispatchers.IO)`는 "저장을 시작하고 잊는다"는 뜻이고, 그래서 완료 시점을 알 수 없다. 이 성질이 스낵바 타이밍 문제로 이어진다. → [`048-android-viewmodel-encapsulation-and-async-timing.md`](048-android-viewmodel-encapsulation-and-async-timing.md)
